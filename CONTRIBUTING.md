# Contributing to SAP SuccessFactors Browser Automation
...
- **Broken selectors**: SAP SuccessFactors updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real SAP SuccessFactors instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SUCCESSFACTORS_URL}/path/to/section`);
...
When you find working selectors for SAP SuccessFactors, document them:

```javascript
// SAP SuccessFactors selectors (verified YYYY-MM-DD)
...
SAP SuccessFactors updates its UI periodically. When selectors break:
