/**
 * anchor-cloud.js — AnchorBrowser cloud example for SAP SuccessFactors
 *
...
const { login_sf, run_workforce_report } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting SAP SuccessFactors automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_sf(page, {});
      log('Action 1 result:', data1);

      const data2 = await run_workforce_report(page, {});
